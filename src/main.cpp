...
// Loot Filter after 0.12.6 Wipe
#include "coolitems.hpp"
#include "realcoolitems.hpp"
#include "questitems.hpp"
...
int SDL_main(int argc, char* argv[])
{
    ...
    //Window Name
    SDL_Window* win = SDL_CreateWindow("Lastest Version 0.12.6.7679", 100, 100, 1024, 768, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
            ...
            if (tk::Observer* player = tk::g_state->map->get_player(); player)
            {
                ...
                for (tk::Observer* obs : tk::g_state->map->get_observers())
                {
                    ...
                    else
                    {
                        ...
                        if (obs->type == tk::Observer::Scav && obs->is_npc && obs->name._Equal("Scavassault")) // Regular Scav Gelb
                        {
                            r = 255;
                            g = obs->is_dead ? 140 : 255;
                        }
                        else if (obs->type == tk::Observer::Scav && !obs->is_npc) // Playerscav Pink
                        {
                            r = 255;
                ...
                auto draw_loot = [&](tk::LootEntry* entry, bool include_equipment = true)
                {
                    ...
                    if (coolitems.count(entry->name)) //red
                    {
                        r = 255;
                        g = 0;
                        b = 0;
                    ...
                    if (realcoolitems.count(entry->name)) //blue
                    {
                        r = 0;
                        g = 0;
                        b = 255;
                    ...
                    if (questitems.count(entry->name)) //brown
                    {
                        r = 105;
                        g = 39;
                        b = 28;
                        ...
                        draw_beam = true;
                        draw_text = true;
                        name = questitems[entry->name].c_str();
                    }
                    ...
                    if (float distance = glm::length(entry_pos - player_pos); distance >= MIN_DISTANCE_FOR_LOOT)
                    {
                        if (draw_text)
                        {
                            ...
                            //loot_text_to_render.push_back(std::make_tuple(entry->pos, coolitems[entry->name].c_str(), 255, 0, 0));
                        }
                ...
                {
                    draw_text(pos.x, pos.y + 22.0f, pos.z, 0.75f, txt.c_str(), r, g, b, get_alpha_for_y(player_y, pos.y), &view, &projection);
                }
